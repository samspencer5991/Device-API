...
void sendOkPacket(uint8_t transport)
{
	...
	tud_cdc_n_write(0, (uint8_t*)buffer, strlen(buffer));
	tud_cdc_n_write_flush(0);
}
...
void sendEventPacket(char* message, uint8_t transport)
{
	tud_cdc_n_write(0, message, strlen(message));
	tud_cdc_n_write_flush(0);
}
...
DeviceApiState deviceApi_Handler(char* appData, uint8_t transport)
{
	...
	else
	{
		...
		while(1)
		{
			...
			{
				// Check the number of bytes available and check for a potential buffer overflow
				uint32_t numBytes = tud_cdc_n_available(0);
				if((byteCount + numBytes) > USB_DATA_LEN)
				{
				...
				{
					numBytes = tud_cdc_n_read(0, &appData[byteCount], USB_DATA_LEN);
					//tud_cdc_n_read_flush(0);
					byteCount += numBytes;
				}

				// Check for an invalid '\0' character terminator
				if(appData[byteCount-1] == '\0')
				{
					sendInvalidTerminationPacket(transport);
					resetDeviceApi();
					return DeviceApiError;
				...
				else if(appData[byteCount-1] == termChar[0])
				{
					// Terminate the string correctly
					appData[byteCount-1] = '\0';
					if(appData[18] == 53 || appData[17] == 53 || appData[19] == 53)
					{
						HAL_Delay(0);
					}
